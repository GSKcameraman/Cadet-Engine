...
static void homework2_test_extra(heap_t* heap, fs_t* fs) {
	
	char* write_data = "Manifesto of the Communist Party\nA spectre is haunting Europe — the spectre of communism.All the powers of old Europe have entered into a holy alliance to exorcise this spectre: Pope and Tsar, Metternichand Guizot, French Radicalsand German police - spies.\nWhere is the party in opposition that has not been decried as communistic by its opponents in power ? Where is the opposition that has not hurled back the branding reproach of communism, against the more advanced opposition parties, as well as against its reactionary adversaries ?\nmTwo things result from this fact :\nI.Communism is already acknowledged by all European powers to be itself a power.\nII.It is high time that Communists should openly, in the face of the whole world, publish their views, their aims, their tendencies, and meet this nursery tale of the Spectre of Communism with a manifesto of the party itself.\nTo this end, Communists of various nationalities have assembled in Londonand sketched the following manifesto, to be published in the English, French, German, Italian, Flemishand Danish languages.";
	const size_t sizew = strlen(write_data);
	fs_work_t* write_work = fs_write(fs, "Communist1.bar", write_data, sizew, true);
	fs_work_t* read_work = fs_read(fs, "Communist1.bar", heap, true, true);

	assert(fs_work_get_result(write_work) == 0);
	assert(fs_work_get_size(write_work) == sizew);

	char* read_data = fs_work_get_buffer(read_work);
	assert(read_data && strcmp(read_data, write_data) == 0);
	assert(fs_work_get_result(read_work) == 0);
	assert(fs_work_get_size(read_work) == sizew);
